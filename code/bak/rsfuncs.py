...
def monthly_sum(dataset, years, months, area):
	...
	for year in years:
		...
		for month in months:
			...
			monthly.append(r)
			time.sleep(1) # so ee doens't freak out 
...
def calc_monthly_sum(dataset, years, months, area):
	...
	'''
	Calculates monthly sum for RS data. Used for Precip data, 
	Units in: mm / time 
	Units out: km/^3 / month
	'''
	...
	for i in num_steps:
		...
		num_ims = ImageCollection.filterDate(start, end).size()
		im = ee.ImageCollection(ImageCollection).select(var).filterDate(start, end).sum().set('system:time_start', start.millis())
		ic = im.multiply(1e-3).multiply(ee.Image.pixelArea()).multiply(scaling_factor).multiply(1e-9).multiply(num_ims)
...
def calc_monthly_mean(dataset, years, months, area):
	
	'''
	Calculates monthly mean for RS data. Used for ET, SM, 
	Units in: kg / m^2 / s 
	Units out: km/^3 / month
	'''
	ImageCollection = dataset[0]
	...
	for i in num_steps:
		...
		num_ims = ImageCollection.filterDate(start, end).size()
		print(num_ims.getInfo())
		im = ee.ImageCollection(ImageCollection).select(var).filterDate(start, end).sum().set('system:time_start', start.millis())
		ic = im.multiply(1e-3).multiply(ee.Image.pixelArea()).multiply(scaling_factor).divide(num_ims).multiply(1e-9)
		scale = ic.projection().nominalScale()
		
		meandict  = ic.reduceRegion(
			reducer = ee.Reducer.sum(),
			geometry = area,
			scale = scale,
			bestEffort = True)

		total = meandict.getInfo()[var]
...
def load_data():
	...
	(1) ImageColection
	(2) variable name
	(3) scale factor - needed to calculate volumes when computing estimators. Depends on units and sampling frequency 
	(4) native resolution -  to return gridded images from `get_ims` function
	...
	Provider's Note: the names with extension _tavg are variables averaged over the past 3-hours, t
	he names with extension '_acc' are variables accumulated over the past 3-hours, 
	the names with extension '_inst' are instantaneous variables, 
	and the names with '_f' are forcing variables.

	
	'''
	# FLDAS: Monthly, 10km
	# https://developers.google.com/earth-engine/datasets/catalog/NASA_FLDAS_NOAH01_C_GL_M_V001
	data['fldas_aet'] = [ee.ImageCollection('NASA/FLDAS/NOAH01/C/GL/M/V001'), "Evap_tavg", 24*60*60, 12500] # Seconds in a month 

	# GLDAS: 3-hourly, 25 km
	data['gldas_aet'] = [ee.ImageCollection('NASA/GLDAS/V021/NOAH/G025/T3H'), 'Evap_tavg',24*60*60 / 3, 25000] # 3 hour periods per day
	data['gldas_pet'] = [ee.ImageCollection('NASA/GLDAS/V021/NOAH/G025/T3H'), 'PotEvap_tavg', 0.408, 25000]  # W/m^2 to mm 

	# NLDAS: hourly, 12.5km
	# https://developers.google.com/earth-engine/datasets/catalog/NASA_NLDAS_FORA0125_H002
	data['nldas_pet'] = [ee.ImageCollection('NASA/NLDAS/FORA0125_H002'), 'potential_evaporation', 1] # hourly cadence 

	# MODIS: 8-day, 500m
	# https://developers.google.com/earth-engine/datasets/catalog/MODIS_006_MOD16A2
	data['modis_aet'] = [ee.ImageCollection('MODIS/006/MOD16A2'), "ET", 0.1, 500]
	data['modis_pet'] = [ee.ImageCollection('MODIS/006/MOD16A2'), "PET", 0.1, 500]
	...
	data['gmet_etr'] = [ee.ImageCollection('IDAHO_EPSCOR/GRIDMET'), "etr", 1, 4000]
	data['gmet_eto'] = [ee.ImageCollection('IDAHO_EPSCOR/GRIDMET'), "eto", 1, 4000]

	###################
	##### P data ######
	###################

	data['trmm']  =  [ee.ImageCollection('TRMM/3B43V7'), "precipitation", 1, 25000]
